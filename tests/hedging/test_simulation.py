...
from quantlab.hedging.simulation import (
    ...
    simulate_heston_hedging_path_with_bs_delta,
)
...
def test_simulate_heston_hedging_path_with_bs_delta_basic():
    """Test basic functionality of hedging path simulation using BS delta."""
    # Set up parameters
    market_state = MarketState(stock_price=100.0, interest_rate=0.05, time=0.0)
    params = HestonParameters(v0=0.04, kappa=2.0, theta=0.04, eta=0.3, rho=-0.7)
    process = HestonProcess(params, market_state)
    ...
    # Use ATM implied vol for BS hedging
    atm_iv = 0.20  # 20% ATM vol

    # Run simulation with BS delta
    pnl, stock_path, var_path, delta_path = simulate_heston_hedging_path_with_bs_delta(
        option=option,
        process=process,
        initial_price=100.0,
        bs_iv=atm_iv,
...
def test_simulate_heston_hedging_path_with_bs_delta_deterministic():
    """Test that the same seed produces the same result with BS delta."""
    market_state = MarketState(stock_price=100.0, interest_rate=0.05, time=0.0)
    params = HestonParameters(v0=0.04, kappa=2.0, theta=0.04, eta=0.3, rho=-0.7)
    process = HestonProcess(params, market_state)
    ...
    # Run twice with same seed
    pnl1, sp1, vp1, dp1 = simulate_heston_hedging_path_with_bs_delta(
        option=option,
        process=process,
        initial_price=100.0,
        bs_iv=atm_iv,
    ...
    pnl2, sp2, vp2, dp2 = simulate_heston_hedging_path_with_bs_delta(
        option=option,
        process=process,
        initial_price=100.0,
        bs_iv=atm_iv,
...
@pytest.mark.slow
def test_simulate_heston_hedging_path_with_bs_delta_vs_heston_delta():
    """Test that BS delta hedging gives different P&L than Heston delta hedging."""
    ...
    # Build delta interpolator for Heston hedging
    S_grid = np.linspace(80, 120, 5)
    T_grid = np.linspace(0.1, 1.0, 5)
    delta_interp = build_delta_interpolator(
        option=option,
    ...
    # Run both simulations with same seed and same Heston paths
    pnl_heston_delta, _, _, _ = simulate_heston_hedging_path(
        option=option,
        process=process,
        initial_price=100.0,
        delta_interpolator=delta_interp,
    ...
    pnl_bs_delta, _, _, _ = simulate_heston_hedging_path_with_bs_delta(
        option=option,
        process=process,
        initial_price=100.0,
        bs_iv=atm_iv,
        n_steps=10,
        seed=42,  # Same seed = same Heston path
    )

    assert isinstance(pnl_heston_delta, (float, np.floating))
    assert isinstance(pnl_bs_delta, (float, np.floating))

    # Both should be finite
    assert np.isfinite(
        pnl_heston_delta
    ), f"Heston delta P&L should be finite, got {pnl_heston_delta}"
    assert np.isfinite(
        pnl_bs_delta
    ), f"BS delta P&L should be finite, got {pnl_bs_delta}"
